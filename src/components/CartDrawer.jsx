...
export default function CartDrawer() {
    ...
    useEffect(() => {
        const handleWarning = (e) => {
            if (e.detail?.message) {
                setStockWarning(e.detail.message)
                setTimeout(() => setStockWarning(''), 4000)
            }
        }
        window.addEventListener('cart-stock-warning', handleWarning)
    ...
    return (
        <div className={`fixed inset-0 z-[150] flex justify-end font-sans transition-opacity duration-300 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
            ...
            <div className="relative w-full max-w-md h-full bg-white shadow-2xl flex flex-col z-10 animate-[slideInRight_300ms_cubic-bezier(0.16,1,0.3,1)]">
                ...
                {/* Stock Warning Toast */}
                {stockWarning && (
                    <div className="bg-amber-50 border-b border-amber-200 p-3 text-center text-xs font-bold text-amber-800 animate-[fadeIn_150ms_ease-out] flex items-center justify-center gap-2">
                        ...
                        <>
                            ...
                            <div className="space-y-4">
                                {cart.filter(item => item && item.id).map((item) => (
                                    <div key={`${item.id}-${item.size}-${item.color || ''}-${item.customText || ''}-${item.isKit ? 'kit' : 'norm'}-${item.kitName || ''}`} className="flex gap-3 pb-4 border-b border-gray-100 last:border-0">
                                        {/* Thumbnail */}
                                        <div className="w-16 h-20 sm:w-20 sm:h-24 bg-gray-50 rounded-lg overflow-hidden flex-shrink-0 border border-gray-100">
                                            <img 
                                        ...
                                        <div className="flex-1 flex flex-col justify-between py-0.5">
                                            <div>
                                                <div className="flex justify-between gap-2">
                                                    <h3 className="font-heading text-xs sm:text-sm font-semibold text-[#1A1A1A] line-clamp-1">
                                                        {item.name}
                                                    </h3>
                                                    <button 
                                                ...
                                                <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-0.5">
                                                    <p className="text-[11px] text-gray-400">Tamanho: <span className="font-bold text-[#1A1A1A] uppercase">{item.size}</span></p>
                                                    {item.color && (
                                                        <p className="text-[11px] text-gray-400">Cor: <span className="font-bold text-[#1A1A1A]">{item.color}</span></p>
                                                    )}
                                                </div>
                                                {item.isKit && (
                                                    <div className="mt-1 bg-emerald-50 border border-emerald-200/80 px-2 py-0.5 rounded-lg text-[9px] text-emerald-800 font-bold inline-block">
                                                        🎁 KIT PROMOCIONAL ({item.kitName || 'Kit'})
                                                    </div>
                                                )}
                                                {item.customText && (
                                                    <div className="mt-1 bg-[#7A3E4A]/5 border border-[#7A3E4A]/10 px-2.5 py-0.5 rounded-lg text-[9px] text-[#7A3E4A] font-bold">
                                            ...
                                            <div className="flex items-center justify-between mt-1">
                                                ...
                                                <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-gray-50/50">
                                                    <button 
                                                        ...
                                                        className="px-2 py-0.5 text-gray-500 hover:bg-gray-100 transition-colors text-xs cursor-pointer"
                                                    >
                                                        -
                                                    </button>
                ...
                {/* Footer Section: Summary Costs & Action Button */}
                {cart.length > 0 && (
                    <div className="p-4 sm:p-6 border-t border-gray-100 bg-white space-y-4 shrink-0 shadow-lg z-20">
                        {/* Summary Costs Table */}
